...
def generate_certificates(certificate):
    ...
    if company == 'CSE':
        ...
        style = {
            ...
            'date': {'type': 'simple', 'y': 3100, 'x': 60, 'size': 50},
        }
    else:
        ...
        style = {
            ...
            'number': {'type': 'simple', 'y': 2955, 'x': 120, 'size': 50},
            'date': {'type': 'simple', 'y': 3025, 'x': 200, 'size': 50},
        }
    ...
    def process_image(template_name):
        ...
        def draw_field(text, key, font_path, color):
            ...
            if cfg['type'] == 'autofit_one_line':
                ...
                while current_size >= min_size:
                    font = get_font(font_path, current_size)
                    lines = wrap_text(text, font, cfg['max_width'], draw)
                    total_h = 0
                    for line in lines:
                ...
                if not final_font:
                    final_font = get_font(font_path, min_size)
                    final_lines = wrap_text(text, final_font, cfg['max_width'], draw)
                    final_h = 0
                    for line in final_lines:
                        bbox = draw.textbbox((0, 0), line, final_font)
                        final_h += (bbox[3] - bbox[1]) + 15
        ...
        draw_field(certificate.seminar.title, 'title', style['font_main'], style['color_title'])
